% This matlab code computes the ground state energy of carbon (C) atom by
% solving the Kohn-Sham (KS) equation in the local-density-approximation
% (LDA) of the density-functional theory (dft) [1]. 
% The exchange and correlation contribution (E_xc) is computed in the Vosko, Wilk, and
...
%
function [] = C_dft_lda_vwn
%
clear; clc;
format long
%
clear; clc;
Z = 6.;   % charge of berrylium nucleus; 
...
D2 = D2(2:N,2:N);
%
% [En, rho] = H0_eig_val(D2,wr,r,Z,ns,wf_ind,ell)
[En_1s, rho_1s] = H0_eig_val(D2,wr,r,Z,1,1,0); % ns = 1 & 1s orbital
[En_2s, rho_2s] = H0_eig_val(D2,wr,r,Z,2,2,0); % ns = 2 & 2s orbital
[En_2p, rho_2p] = H0_eig_val(D2,wr,r,Z,1,1,1); % ns = 2 & 2p orbital
%
[En_1s, En_2s, En_2p];
%
rho_old = 2.*rho_1s + 2.*rho_2s + 2.*rho_2p; 
%
x_alpha = 3./4;
%
ac1 = 0.0621814/2;
...
x0 = -0.10498;
%
n_xc = 1.; % if n_xc = 0, X-lapha; n_xc = 1, LDA-VWN;
%
for iter = 1:itermax
    %
    iter
...
%
    ...
    [En_2p, rho_2p_] = H_KS_eig_val(D2,wr,r,Z, 1., 1., 1., Vpois_11,V_ex, n_xc.*V_corr); % KS: 2p orbital        
    %
    rho_new = 2.*rho_1s_ + 2.*rho_2s_ + 2.*rho_2p_;
    %
...
%%%
[En_1s, En_2s, En_2p]; % 1s, 2s, 2p orbital energies
%
...
%
En_total_gr_au = (2.*En_1s + 2.*En_2s + 2.*En_2p) - 0.5*Z.*Vee + En_xc ;
%
[En_1s, En_2s, En_2p, En_total_gr_au]
% [-9.947718226571375  -0.500866100291089  -0.199185716806009 -37.425748535984255] vs 
% [-9.947718,          -0.500866,          -0.199186,         -37.425749]
% from https://www.nist.gov/pml/atomic-reference-data-electronic-structure-calculations/atomic-reference-data-electronic-7-4
%%%
