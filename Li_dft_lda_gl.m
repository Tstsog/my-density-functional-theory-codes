...
%
function [] = Li_dft_lda_gl
%
clc;
clear Li_dft_lda_gl
format short 
%
Z = 3.;   % charge of lithium nucleus; 
N = 2*64;   % number of grid point; you may change it
...
%
    ...
    xx = rc./AA;
    eps_c = -c_p.*( (1.+xx.^3).*log(1.+(1./xx)) + 0.5.*xx - xx.^2 - (1./3) ) *(1/2);
    V_corr = -c_p.*log(1. + (1./xx)) *(1/2);
    %%%          
    %
    ...
    %
    rho_new = 2.*rho_1s_ + 1.*rho_2s_ + 0.*rho_2p_;
    %
...
%
En_total_gr_au = (2.*En_1_1s + 1.*En_2_2s) - 0.5*Vee + En_xc ;
%
%
[En_1_1s, En_2_2s, En_total_gr_au] % -0.2416   -0.4537
...
%
%%%
H_ham = -0.5.*D2 + diag(ell*(ell+1)./(2.*r.^2)) - diag(Z./r) + 1.*diag(Vpois_11) + 1.*diag(V_ex) + 1.*diag(V_corr);
[Vec,En] = eig(H_ham);                                     % Eigenvalue problem
En = diag(En);
[foo, ij] = sort(En);
En = En(ij);
