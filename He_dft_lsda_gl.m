...
%
function [] = He_dft_lsda_gl
%
clc;
clear He_dft_lsda_gl
format short 
%
...
[En_2p, V2p, rho_2p] = H0_eig_val(D2,wr,r,Z,1,1.); % 2p
%
[En_1s, En_2s, En_2p]
%
%rho_1s_old = 2.*rho_1s + 2.*rho_2s + 1.*rho_2p; 
%
gamma = 0.297;
Ry = 0.5; % Ry = 0.5au
alpha = (4/(9*pi))^(1/3);
...
V_xc = V_xc_alpha + 1.*V_xc_beta;
%
Vee = sum(wr.*rho_1s_old.*Vpois_11.*r.*r.*4.*pi);
En_xc = sum(wr.*rho_1s_old.*eps_xc.*r.*r.*4.*pi);
%
En_total_gr_au = (En_1s_alpha + 1.*En_1s_beta) - 0.5.*Vee + (En_xc - V_xc); % -2.860137154386072
%
[En_1s_alpha, En_1s_beta, En_total_gr_au] % -0.5825   -0.5825   -2.8601
